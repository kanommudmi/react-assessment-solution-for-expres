...
export function Navbar({ user, authLoading, authError, login, logout }) {
  ...
  const handleSubmit = async (e) => {
    // block the form dont refresh page
    e.preventDefault();
    setSummitting(true);
        ...
        <div className="flex items-center gap-x-3">
          {authLoading ? (
            ...
            <form onSubmit={handleSubmit} className="flex items-center gap-x-2">
              ...
              />
              <button type="submit" disabled={summitting} className="cursor-pointer bg-sky-500 hover:bg-sky-600 disabled:bg-sky-300 text-white px-3 py-1 rounded-xl text-base">
                Login
              </button>
            </form>
          )}
